var HttpClient, ReplaceNumberWithCommas, addToHistory, current_song, dont_play, isPlaying, newSong, onPlayerReady, onPlayerStateChange, onYouTubeIframeAPIReady, pauseVideo, player, randSong, resize, songDataReady, song_data, song_history, startVideo, stopVideo, url_params, __indexOf = [].indexOf
		|| function(item) {
...
url_params = [ "enablejsapi=1", "controls=0", "showinfo=0", "modestbranding=0",
		"autoplay=1", "cc_load_policy=0", "iv_load_policy=3",
		"origin=http://www.jombly.com/", "playsinline=1", "disablekb=1",
		"fs=0", "rel=0", "wmode=transparent" ].join("&");
...
songDataReady = function() {
	...
	for (_i = 0, _len = song_data.length; _i < _len; _i++) {
		...
		if (_ref = song.query, __indexOf.call(dont_play, _ref) < 0) {
			$("#topList ol").append(
					"<li class='topSong' data-song=" + song.rank + "> <strong>"
							+ song.artist + "</strong> / <em>" + song.title
							+ "</em> </li>");
		}
	}
	for (_j = 0, _len1 = dont_play.length; _j < _len1; _j++) {
		...
		$("#badList ol").append(
				"<li class='badSong' data-song=" + song.rank + "> <strong>"
						+ song.artist + "</strong> / <em>" + song.title
						+ "</em> </li>");
	}
};
...
$("#dontPlay")
		.on(
				...
				function() {
					var _ref;
					if (_ref = current_song.query, __indexOf.call(dont_play,
							_ref) < 0) {
						dont_play.push(current_song.query);
						localStorage.setItem("dontPlay", JSON
								.stringify(dont_play));
						$("#badList ol").append(
								"<li class='badSong' data-song="
										+ current_song.rank + "> <strong>"
										+ current_song.artist
										+ "</strong> / <em>"
										+ current_song.title + "</em> </li>");
						$(
								"#topList ol > li:nth-child("
										+ current_song.rank + ")").remove();
						return newSong();
					}
				});
...
$('#badList').on(
		...
		function() {
			...
			return $("#topList ol > li:nth-child(" + id + ")").before(
					"<li class='topSong' data-song=" + song.rank + "> <strong>"
							+ song.artist + "</strong> / <em>" + song.title
							+ "</em> </li>");
		});
...
$(document)
		.ready(
				function() {
					...
					$
							.getJSON(
									"http://graph.facebook.com/?id=http://www.jombly.com",
									function(fbdata) {
										$("#facebook-count")
												.text(
														ReplaceNumberWithCommas(fbdata.shares));
									});
					$
							.getJSON(
									"http://cdn.api.twitter.com/1/urls/count.json?url=http://www.jombly.com&callback=?",
									function(twitdata) {
										$("#twitter-count")
												.text(
														ReplaceNumberWithCommas(twitdata.count));
									});
